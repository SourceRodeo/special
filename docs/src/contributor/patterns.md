...
@implements SPECIAL.DOCUMENTATION.CONTRIBUTOR.PATTERNS
...
| Pattern | Use it when |
| --- | --- |
| [`ADAPTER.FACTS_TO_MODEL`](documents://pattern/ADAPTER.FACTS_TO_MODEL) | provider-specific facts must become shared Special model data |
| [`ADAPTER.FACTS_TO_MODEL.TRACEABILITY_ITEMS`](documents://pattern/ADAPTER.FACTS_TO_MODEL.TRACEABILITY_ITEMS) | a language pack projects parsed items into shared traceability items |
| [`TRACEABILITY.SCOPED_PROJECTED_KERNEL`](documents://pattern/TRACEABILITY.SCOPED_PROJECTED_KERNEL) | scoped traceability must preserve full-then-filtered semantics |
| [`TEST_FIXTURE.REPRESENTATIVE_PROJECT`](documents://pattern/TEST_FIXTURE.REPRESENTATIVE_PROJECT) | a test needs a compact real project instead of a source-string snippet |
| [`COMMAND.PROJECTION_PIPELINE`](documents://pattern/COMMAND.PROJECTION_PIPELINE) | a command renders a read-only projection from parsed source state |
| [`SINGLE_FLIGHT.CACHE_FILL`](documents://pattern/SINGLE_FLIGHT.CACHE_FILL) | expensive cache fills need one concurrent producer per cache key |
| [`REGISTRY.PROVIDER_DESCRIPTOR`](documents://pattern/REGISTRY.PROVIDER_DESCRIPTOR) | built-in providers need one capability descriptor surface |
